...
def award_is_returned(entry: dict) -> bool:
    """Return True when an award has been returned to the clan."""
...
def get_filtered_roll_awards(
    ...
    for entry in award_log:
        ...
        if not include_returned and award_is_returned(entry):
            continue
    ...
    if include_returned:
        # Audit is easiest to follow newest-first.
        return sorted(
            entries,
            key=lambda entry: (
    ...
    # Current ownership is easier to scan alphabetically by item, then player.
    return sorted(
        entries,
        key=lambda entry: (
...
def get_active_award_for_channel(channel_id: int, guild_id: int) -> dict | None:
    """Return the newest active award associated with a channel/thread."""
    matches = [
        entry
        for entry in award_log
        if int(entry.get("guild_id", 0)) == guild_id
...
def build_roll_audit_content(
    ...
    lines.append(f"History entries: **{total}** • Page **{page + 1}/{total_pages}**")

    if not entries:
        lines.extend(["", "No audit entries matched those filters."])
        return "\n".join(lines)
    ...
    for entry in page_entries:
        ...
        status = "RETURNED" if award_is_returned(entry) else "ACTIVE"

        lines.extend(
            [
                "",
                f"**#{log_id} — {item}** • **{status}**",
                f"Winner: <@{winner_id}> • Saved as: **{stored_name}**",
                f"Roll: **{roll_value}** • {method_label} • Awarded {awarded_at}",
                f"Roll ID: `{roll_id}`",
            ]
        )

        if award_is_returned(entry):
            returned_at = format_award_timestamp(entry.get("returned_at"))
            returned_by = entry.get("returned_by_user_id")
            if returned_by:
                lines.append(f"Returned: {returned_at} • By <@{int(returned_by)}>")
            else:
                lines.append(f"Returned: {returned_at}")
...
def build_roll_awards_embed(
    ...
    if not entries:
        ...
        for offset, entry in enumerate(page_entries, start=start + 1):
            ...
            if member_id is not None:
                ...
                lines.append(f"**{offset}.** {item} — <@{winner_id}>")
    ...
    if member_id is not None and entries:
        footer += " • Leaders: Return Item → enter list #"
...
def build_roll_audit_embed(
    ...
    embed = discord.Embed(title="📋 Roll Audit")
    ...
    if not entries:
        embed.add_field(
            ...
            value="No audit entries matched those filters.",
            inline=False,
        )
    else:
        start = page * per_page
        ...
        for entry in page_entries:
            ...
            returned = award_is_returned(entry)
            status = "↩️ RETURNED" if returned else "✅ ACTIVE"

            value_lines = [
                f"**Holder:** <@{winner_id}> ({stored_name})",
            ...
            if returned:
                ...
                if returned_by:
                    value_lines.append(
                        f"**Returned:** {returned_at} by <@{int(returned_by)}>"
                    )
                else:
                    value_lines.append(f"**Returned:** {returned_at}")

            embed.add_field(
                name=f"#{log_id} • {status} • {item}",
                value="\n".join(value_lines),
                inline=False,
            )

    embed.set_footer(text=f"{total} audit entries • Page {page + 1}/{total_pages}")
    return embed
...
def build_roll_panel_content(state: dict, roll_id: int) -> str:
    ...
    if state.get("award_recorded"):
        ...
        recorded_award = get_award_for_roll(roll_id)
        was_returned = (
...
async def process_award_return(
    ...
    """Mark an award returned, preserve audit history, and refresh its roll panel."""
    ...
    target_award["returned_by_name"] = getattr(
        actor,
        "display_name",
        getattr(actor, "name", "Unknown"),
    )
    ...
    # Refresh the original roll panel so the item thread keeps the full trail.
    if state is not None:
        channel_id = state.get("channel_id")
        roll_channel = bot.get_channel(channel_id) if channel_id else None
        ...
        if roll_channel is not None:
            try:
                roll_message = await roll_channel.fetch_message(roll_id)
...
class RollAwardsView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        await interaction.response.send_message(
            "Run `%rollawards` yourself to browse this list.",
            ephemeral=True,
        )
        return False
    ...
    *,
    ...
    if winner is None:
        ...
        if selected_roll is None:
            # Leader override: the item can be awarded to someone who did not
            # participate in the original roll (for example, everyone passed
            # and an unclaimed item was given out afterward).
            selected_roll = {
                "user_id": winner.id,
            ...
            selection_method = "manual_no_roll"
        else:
    ...
    if selection_method == "manual_no_roll":
        method_text = "manual award — did not roll"
    elif selection_method == "manual":
        method_text = "manually selected"
    else:
        method_text = "highest roller"

    roll_display = selected_roll.get("roll")
    if roll_display is None:
        roll_display = "N/A"
    ...
    embed.add_field(name="Winner", value=f"<@{winner_user_id}>", inline=True)
    embed.add_field(name="Roll", value=f"{roll_display} ({method_text})", inline=True)
    embed.add_field(name="Award", value=f"#{log_id}", inline=True)
    embed.set_footer(text=f"Roll ID: {parsed_roll_id}")
