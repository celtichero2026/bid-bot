...
class RollAuditView(discord.ui.View):
    ...
    def content(self) -> str:
        return build_roll_audit_content(
            self.entries,
            self.page,
            self.per_page,
            member_id=self.member_id,
...
async def rollawards(ctx: commands.Context, *, filters: str = ""):
    """Browse current (not returned) roll awards in a clean item list."""
...
async def rollaudit(ctx: commands.Context, *, filters: str = ""):
    ...
    content = build_roll_audit_content(
        entries,
        page=0,
        per_page=6,
        member_id=member_id,
...
async def returnaward(ctx: commands.Context, *, arguments: str = ""):
    ...
    Usage:
      ...
      %returnaward <award_log_id>      -> exact award from anywhere
      %returnaward @Player <item name> -> find a player's current item
    """
    if ctx.guild is None or not is_leader(ctx.author, ctx.guild):
    ...
    if not raw:
        ...
        if target_award is None:
            await ctx.send(
                ...
                "Use `%returnaward @Player item name` or `%returnaward <award #>`."
            )
            return
        ...
        if log_id_text.isdigit():
            ...
            if member is None:
                await ctx.send(
                    ...
                    "`%returnaward <award #>`, or `%returnaward @Player item name`."
                )
                return
            ...
            if not item_query:
                await ctx.send(
                    "Include the item name, for example `%returnaward @Player Dhio Bow`."
                )
                return
            ...
            if len(matches) > 1:
                choices = "\n".join(
                    f"• **#{entry.get('log_id', '?')}** — "
                    f"{discord.utils.escape_markdown(str(entry.get('item', 'Unknown item')))}"
                    for entry in matches[:8]
                )
                await ctx.send(
                    "That matches more than one current award. Use the exact award "
                    "number from `%rollaudit`, for example `%returnaward 12`.\n\n"
                    f"{choices}",
                    allowed_mentions=discord.AllowedMentions.none(),
                )
                return
    ...
    target_award["returned_by_user_id"] = ctx.author.id
    ...
    if state is not None:
        ...
        state["award_returned_by"] = ctx.author.id

    save_state()

    # Refresh the original roll panel so the thread also shows the return.
    if state is not None:
        channel_id = state.get("channel_id")
    ...
    await ctx.send(
        f"↩️ **Award #{log_id} Returned — {item}**\n"
        f"Previous holder: <@{winner_id}>\n"
        "Removed from `%rollawards` and kept in `%rollaudit` as returned.",
        allowed_mentions=discord.AllowedMentions.none(),
    )
